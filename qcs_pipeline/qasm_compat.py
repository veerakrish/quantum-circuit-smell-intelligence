...
them anyway via a hardcoded legacy gate list. Every circuit in this dataset
was dumped with that old, lenient exporter, so strict parsing fails on every
single one with "'sx' is not defined in this scope" (or similar for other
legacy-only gates) unless we opt back into that same tolerance.
